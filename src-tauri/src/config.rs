...
pub fn get_config() -> MacchaSettings {
	let config_dir = get_config_directory();
	let config_file_path = config_dir.join("config.json");
	let mut config_file = File::open(&config_file_path);

	if let Err(_) = config_file {
		fs::write(&config_file_path, "{}").unwrap_or_else(|_| {
			panic!("Failed to create config file. Please create one manually at \"{:?}\"",
			config_file_path)
		});
...
pub fn get_config_directory() -> PathBuf {
	match home_dir() {
		Some(home) => {
			let dir = Path::new(&home).join(".config").join("maccha");
			fs::create_dir_all(&dir).unwrap_or_else(|_| {
				panic!("Failed to create config directory at \"{:?}\"", dir)
			});
			dir
		}
		None => {
